...
4. **Run verification** using `task-verification` procedures
5. **Track progress** by updating your `task.md` artifact at each state change.
...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "Code quality approved?" [shape=diamond];
        "Mark task complete in task.md artifact" [shape=box];
    }
    ...
    "Fix quality issues" -> "Run code quality review (./code-quality-reviewer-prompt.md)    "Fix issues" -> "Run task-specific verification again";
    "Code quality approved?" -> "Mark task complete in task.md artifact" [label="yes"];
    "Mark task complete in task.md artifact" -> "More tasks remain?";
...
- **What remains** — remaining tasks, knowRun tests to confirm.
Update your `task.md` artifact with current status.
...
[Extract all 5 tasks with full System setup verified.
[Update your `task.md` artifact with all tasks as 'not_started']
...
Code review: Strengths: Good test coverage, clean. Issues: None. ApprBoth passed.
[Mark Task 1 complete in your `task.md` artifact]
...
CodType errors fixed. Build passes.
[Mark Task 2 complete in your `task.md` artifact]
...
Before calling `notify_user` or considering work finished:

1. Ensure all task entries in your `task.md` artifact are `done` or `cancelled`
